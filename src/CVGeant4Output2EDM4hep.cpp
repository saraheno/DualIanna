...
//
//==========================================================================
#ifndef DD4HEP_DDG4_CVGEANT4OUTPUT2EDM4hep_H
#define DD4HEP_DDG4_CVGEANT4OUTPUT2EDM4hep_H
...
namespace dd4hep {
  ...
  namespace sim {
     ...
     */
    class CVGeant4Output2EDM4hep : public Geant4OutputAction  {
    protected:
    ...
    public:
      ...
      CVGeant4Output2EDM4hep(Geant4Context* ctxt, const std::string& nam);
      /// Default destructor
      virtual ~CVGeant4Output2EDM4hep();
...
DECLARE_GEANT4ACTION(CVGeant4Output2EDM4hep)

/// Standard constructor
CVGeant4Output2EDM4hep::CVGeant4Output2EDM4hep(Geant4Context* ctxt, const std::string& nam)
...
/// Default destructor
CVGeant4Output2EDM4hep::~CVGeant4Output2EDM4hep()  {
  G4AutoLock protection_lock(&action_mutex);
  InstanceCount::decrement(this);
}

// Callback to store the Geant4 run information
void CVGeant4Output2EDM4hep::beginRun(const G4Run* run)  {
  G4AutoLock protection_lock(&action_mutex);
...
/// Callback to store the Geant4 run information
void CVGeant4Output2EDM4hep::endRun(const G4Run* run)  {
  saveRun(run);
  saveFileMetaData();
...
void CVGeant4Output2EDM4hep::saveFileMetaData() {
...
/// Commit data at end of filling procedure
void CVGeant4Output2EDM4hep::commit( OutputContext<G4Event>& /* ctxt */)   {
  if ( m_file )   {
    G4AutoLock protection_lock(&action_mutex);
...
/// Callback to store the Geant4 run information
void CVGeant4Output2EDM4hep::saveRun(const G4Run* run)   {
  G4AutoLock protection_lock(&action_mutex);
...
void CVGeant4Output2EDM4hep::begin(const G4Event* event)  {
...
/// Data conversion interface for MC particles to EDM4hep format
void CVGeant4Output2EDM4hep::saveParticles(Geant4ParticleMap* particles)    {
...
/// Callback to store the Geant4 event
void CVGeant4Output2EDM4hep::saveEvent(OutputContext<G4Event>& ctxt)  {
...
/// Callback to store each Geant4 hit collection
void CVGeant4Output2EDM4hep::saveCollection(OutputContext<G4Event>& /*ctxt*/, G4VHitsCollection* collection)  {
  ...
  //  else if( typeid(CalVision::DualCrysCalorimeterHit ) == coll->type().type() ){
